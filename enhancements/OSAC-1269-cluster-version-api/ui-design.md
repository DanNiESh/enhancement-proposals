...
**Cluster list (`ClustersTable.tsx`, tenant- and admin-visible).** Resolving every row's version is a small algorithm, run once per table render — not once per row:

1. Compute the distinct, non-empty `cluster.spec?.version?.name` values referenced by the currently-rendered clusters. If there are none, skip everything below.
2. Call `useClusterVersions({ filter: buildClusterVersionNamesFilter(names) })` once — a targeted `metadata.name` lookup (`this.metadata.name in [...]`), not a lifecycle-wide listing [User].
3. If that call succeeds, build a `Map<string, ClusterVersion>` from the response, keyed by `metadata.name`.
4. Compute which requested names are still missing from the map, cap that list at `CLUSTER_VERSION_FALLBACK_MAX = 20`, and resolve the capped list with `useQueries` (from `@tanstack/react-query`) — one `Get`-by-name query per missing name, built with the same `queryFn`/`queryKey` shape `useClusterVersion` uses internally. Add successful results to the same map.
5. If step 2's call is still loading or failed, skip step 4 entirely — do not treat an empty/stale map as "everything is missing."

Each row then looks up its `cluster.spec?.version?.name` in the final map. Two new columns: **Version** (the resolved `ClusterVersion.spec.version` string, e.g. "4.17.0", falling back to the raw reference name if unresolved — mirroring `ClusterConfigurationCard.tsx`'s existing catalog-item-name fallback) and **Lifecycle** (`ClusterVersionLifecycleLabel`, blank if unresolved).

**Why steps 3–5 are shaped this way, not a plain per-name `Get` loop:**

- **Step 4 uses `useQueries`, not `useClusterVersion(name)` called per name in a loop or `.map()`.** The latter would call a variable number of hooks across renders, violating the Rules of Hooks. `useQueries` is a single hook call whose *array argument* may vary in length across renders — the supported way to do this. It's a narrow, deliberate exception to this codebase's "use `useApiQuery`, not `@tanstack/react-query` directly" convention: that convention's ESLint rule restricts `useQuery`/`useQueryClient` by name, not `useQueries`, and there is no batched equivalent of `useApiQuery` today.
- **Step 5's gate on `List`'s success exists to prevent a fan-out spike.** Without it, a failed or still-loading `List` call would make every referenced name look "missing" (the map is empty), turning one slow/failed request into up to N concurrent `Get` requests — the opposite of what a struggling backend needs. A failed `List` instead falls through to the existing "List call fails or is slow" handling in §4.6, same as today.
- **Step 4's `CLUSTER_VERSION_FALLBACK_MAX = 20` cap exists because step 2's filter leaves one thing unconfirmed.** `buildClusterVersionNamesFilter()` is deliberately not a lifecycle/enablement filter — that avoids the tautology problem of the rejected all-states alternative (see §5) — but it doesn't confirm whether the public `List` RPC's default disabled/obsolete hiding still applies to a `metadata.name`-only filter: that hiding rule keys off which *fields* the filter touches, and this filter touches neither `spec.state` nor `spec.enabled`. If the hiding turns out to still apply broadly, step 4 could otherwise fan out to one `Get` per distinct referenced version on every page load; the cap bounds that to a small, fixed number regardless of how many names are actually missing. Names beyond the cap use the existing raw-reference-name-with-no-label fallback, the same as a genuinely unresolvable entry. This fallback (even uncapped) is a fundamentally different cost profile from the rejected "per-row `Get` for every cluster" alternative in §5, since it only fires for entries the batched `List` call didn't already resolve.

Whether the name filter already returns disabled/obsolete entries on its own (making step 4 a safety net that never fires) or not (making step 4 load-bearing) must be observed during implementation, not assumed — see §4.3 and Story 2.01's acceptance criteria.
