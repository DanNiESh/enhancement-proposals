...
- **Mark deprecated / Mark obsolete / Reactivate** — calls `useSetClusterVersionLifecycleState()`, a mutation wrapping the same `update` RPC as Edit but scoped to `spec.state` only, kept as a separate hook so lifecycle transitions are a distinct, auditable action from general field edits. Available actions are computed from the current state per the backend's state machine (e.g., an `OBSOLETE` entry offers "Reactivate" and "Mark deprecated", not "Mark obsolete") [Codebase: `design.md` lifecycle `stateDiagram-v2`].
