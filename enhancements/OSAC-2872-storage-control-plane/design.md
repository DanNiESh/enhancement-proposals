...
**Vendor volume creation failure:** The operator Volume controller retries via the standard `provisioning.RunProvisioningLifecycle()` pattern (backoff, retry, status update). The volume name is derived deterministically from the PVC name, so the vendor CSI CreateVolume is idempotent per the CSI spec.
